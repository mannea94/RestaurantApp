...
import com.example.manne.restaurantapp.Model.MenuData;
import com.google.gson.Gson;

/**
 * Created by manne on 23.12.2017.
 */

public class PreferencesManagerMenu {
    public static void addMenu(MenuData user, Context c){
        Gson gson = new Gson();
        String mapString = gson.toJson(user);
        getPreferences(c).edit().putString("menu", mapString).apply();
    }

    public static MenuData getMenu(Context c){
        return new Gson().fromJson(getPreferences(c).getString("menu", meni), MenuData.class);
    }
    ...
    public static String meni = "{\"restaurants\": [\n" +
            "  {\n" +
